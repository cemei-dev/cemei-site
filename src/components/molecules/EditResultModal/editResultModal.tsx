...
import { updateActionResult } from "@/store/services/result";
import {
  AddActionResultForm,
  AddActionResultSchema
} from "@/validations/addResult";

import { EditActionResultModalProps } from "./types";
...
export default function EditResultModal({
  ...
  }, [result.imageUrl, isOpen, setValue]);
  ...
  const addMutation = useMutation(
    async (data: AddActionResultForm) => {
      ...
      if (data.imageUrl) {
        ...
        try {
          await updateActionResult(result.id, {
            ...data,
            imageUrl
          });
        } catch (error) {
          ...
          errorToast(
            "Erro ao atualizar resultado da ação. Por favor, tente novamente."
          );
          throw error;
        }
      }
    },
    {
      onSuccess: () => {
        ...
        setLoading(false);
        setIsOpen(false);
        successToast("Resultado da ação atualizada com sucesso");
      },
      onError: () => {
        setLoading(false);
      }
